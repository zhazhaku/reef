...
func onboard(encrypt bool) {
	// Ensure reef home directory exists. On fresh installs, this prefers
	// the exe directory (portable mode) over ~/.reef, so that onboard
...
// setupSSHKey generates the reef-specific SSH key at ~/.ssh/reef_ed25519.key.
