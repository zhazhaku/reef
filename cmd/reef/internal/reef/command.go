...
func newReefServerCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(_ *cobra.Command, _ []string) error {
			...
			cfg.DefaultTimeoutMs = taskTimeoutMs
			cfg.TimeoutScanIntervalSec = timeoutScanSec
	...
	cmd.Flags().StringVar(&strategy, "strategy", "", "Match strategy: least_load (default), round_robin, affinity")
	cmd.Flags().Int64Var(&taskTimeoutMs, "task-timeout", 0, "Default task timeout in ms (0 = 5min)")
	cmd.Flags().IntVar(&timeoutScanSec, "timeout-scan", 0, "Timeout scanner interval in seconds (0 = 10s)")
